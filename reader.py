...
f.write('CREATE TABLE IF NOT EXISTS cities( `id` int(11) NOT NULL AUTO_INCREMENT, `name` varchar(255) NOT NULL, `slug` varchar(255) NOT NULL, `department_id` int(11) NOT NULL, PRIMARY KEY (`id`), FOREIGN KEY (`department_id`) REFERENCES departments(`id`)) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;;\n\n')
id_department = 0
id_city = 0
for r in data:
    id_department = id_department+1
    for city in data[r]:
        id_city = id_city+1
        f.write('INSERT INTO cities(`id`, `name`, `slug`, `department_id`) VALUES ('+str(id_city)+', "'+city+'", "'+slugify(city)+'", '+str(id_department)+' );\n') # python will convert \n to os.linesep
